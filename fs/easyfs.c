...
int fs_read_file(const char *name, void *buf, uint32_t size) {
    DirEntry *entry = fs_find_file(name);
    if (!entry) return -1;
    uint32_t blocks_to_read = size / BLOCK_SIZE + 1;
    uint32_t file_blocks = entry->file_size / BLOCK_SIZE + 1;
    if (blocks_to_read > file_blocks)
        blocks_to_read = file_blocks;
    for (uint32_t i = 0; i < blocks_to_read; i++)
        fs_read_block(entry->first_data_block + i, (uint8_t*)buf + i * BLOCK_SIZE);
    return entry->file_size;
}

int fs_write_file(const char *name, const void *buf, uint32_t size) {
    DirEntry *entry = fs_find_file(name);
    if (!entry) return -1;
    uint32_t blocks_needed = size / BLOCK_SIZE + 1;
    for (uint32_t i = 0; i < blocks_needed; i++)
        fs_write_block(entry->first_data_block + i, (uint8_t*)buf + i * BLOCK_SIZE);
